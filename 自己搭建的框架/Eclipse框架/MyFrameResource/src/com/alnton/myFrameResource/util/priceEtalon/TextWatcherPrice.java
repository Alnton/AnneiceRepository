 ...
 */
 class TextWatcherPrice implements TextWatcher {
	...
	public TextWatcherPrice(EditText priceEt,int decimalFrontMaxLength,int decimalBackMaxLength,TextWatcherPriceListener textWatcherPriceListener){
	...
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		...
		if(s.toString().length()>decimalFrontMaxLength&&!s.toString().contains(".")){
			...
			if (s.toString().indexOf(".") > decimalFrontMaxLength) {
				...
				this.priceEt.setText(this.stringBuffer.toString());
				//设置光标在最后
				this.priceEt.setSelection(start);
			}
			//小数点的后面有几位数
		...
		if(null!=this.textWatcherPriceListener){
			this.textWatcherPriceListener.onTextChanged(s, start, before, count);
		}
	}
	@Override
	public void beforeTextChanged(CharSequence s, int start, int count,
		...
		if(null!=this.textWatcherPriceListener){
			this.textWatcherPriceListener.beforeTextChanged(s, start, count, after);
		}
	}
	@Override
	public void afterTextChanged(Editable s) {
		if(null!=this.textWatcherPriceListener){
			this.textWatcherPriceListener.afterTextChanged(s);
		}
	}
