...
package com.alnton.myFrameResource.view.ListView.crossDeleteListView;
 ...
 */
public class NoScrollSwipeListViewTouchListener implements View.OnTouchListener
{
    
    private static final int DISPLACE_CHOICE = 80;
    
    private int swipeMode = NoScrollSwipeListView.SWIPE_MODE_BOTH;
    ...
    // Fixed properties
    private NoScrollSwipeListView swipeListView;
    ...
    private int swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_NONE;
    
    private int swipeActionLeft = NoScrollSwipeListView.SWIPE_ACTION_REVEAL;
    
    private int swipeActionRight = NoScrollSwipeListView.SWIPE_ACTION_REVEAL;
     ...
     */
    public NoScrollSwipeListViewTouchListener(NoScrollSwipeListView swipeListView, int swipeFrontView, int swipeBackView)
    {
        ...
        configShortAnimationTime =
            swipeListView.getContext().getResources().getInteger(android.R.integer.config_shortAnimTime);
    ...
    {
        return swipeMode != NoScrollSwipeListView.SWIPE_MODE_NONE;
    }
    ...
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position)
    {
        ...
        if (swipeCurrentAction == NoScrollSwipeListView.SWIPE_ACTION_REVEAL)
        {
            generateRevealAnimate(view, swap, swapRight, position);
        }
        if (swipeCurrentAction == NoScrollSwipeListView.SWIPE_ACTION_DISMISS)
        {
            generateDismissAnimate(parentView, swap, swapRight, position);
        }
        if (swipeCurrentAction == NoScrollSwipeListView.SWIPE_ACTION_CHOICE)
        {
    ...
    public boolean onTouch(View view, MotionEvent motionEvent)
    {
        ...
        NoScrollSwipeListView swipeListView = (NoScrollSwipeListView)view;
        
        if (null != swipeListView && null != swipeListView.myViewPager)
        {
            swipeListView.myViewPager.requestDisallowInterceptTouchEvent(true);
        }
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent))
        {
            case MotionEvent.ACTION_DOWN:
            {
                ...
                swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_NONE;
            ...
            case MotionEvent.ACTION_UP:
            {
                ...
                {
                    if (swipeMode == NoScrollSwipeListView.SWIPE_MODE_LEFT && velocityTracker.getXVelocity() > 0)
                    {
                        velocityX = 0;
                    }
                    if (swipeMode == NoScrollSwipeListView.SWIPE_MODE_RIGHT && velocityTracker.getXVelocity() < 0)
                    {
                        velocityX = 0;
                    }
                }
            ...
            case MotionEvent.ACTION_MOVE:
            {
                ...
                if (swipeMode == NoScrollSwipeListView.SWIPE_MODE_NONE)
                {
                    deltaMode = 0;
                }
                else if (swipeMode != NoScrollSwipeListView.SWIPE_MODE_BOTH)
                {
                    if (opened.get(downPosition))
                    {
                        if (swipeMode == NoScrollSwipeListView.SWIPE_MODE_LEFT && deltaX < 0)
                        {
                            deltaMode = 0;
                        }
                        else if (swipeMode == NoScrollSwipeListView.SWIPE_MODE_RIGHT && deltaX > 0)
                        {
                            deltaMode = 0;
                        }
                    }
                ...
                if (deltaMode > slop && swipeCurrentAction == NoScrollSwipeListView.SWIPE_ACTION_NONE
                    && velocityY < velocityX)
                {
                    swiping = true;
                    ...
                    if (opened.get(downPosition))
                    {
                        ...
                        swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_REVEAL;
                    }
                    else
                    {
                        if (swipingRight && swipeActionRight == NoScrollSwipeListView.SWIPE_ACTION_DISMISS)
                        {
                            swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_DISMISS;
                        }
                        else if (!swipingRight && swipeActionLeft == NoScrollSwipeListView.SWIPE_ACTION_DISMISS)
                        {
                            swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_DISMISS;
                        }
                        else if (swipingRight && swipeActionRight == NoScrollSwipeListView.SWIPE_ACTION_CHOICE)
                        {
                            swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_CHOICE;
                        }
                        else if (!swipingRight && swipeActionLeft == NoScrollSwipeListView.SWIPE_ACTION_CHOICE)
                        {
                            swipeCurrentAction = NoScrollSwipeListView.SWIPE_ACTION_CHOICE;
                        }
                        else
    ...
    public void move(float deltaX)
    {
        ...
        else if (swipeCurrentAction == NoScrollSwipeListView.SWIPE_ACTION_CHOICE)
        {
