package com.alnton.myFrameResource.view.Wheel;
...
public class WheelView extends View
{
    ...
    /** 当前条目中的文字颜色 */
    private static final int VALUE_TEXT_COLOR = R.color.wheel_text_color;
    
    /** 非当前条目的文字颜色 */
    private static final int ITEMS_TEXT_COLOR = R.color.ITEMS_TEXT_COLOR;
    
    /** 顶部和底部的阴影颜色 */
    //    private static final int[] SHADOWS_COLORS = new int[] {0xFF111111, 0x00AAAAAA, 0x00AAAAAA};
    private static final int[] SHADOWS_COLORS = new int[] {android.R.color.transparent, android.R.color.transparent,
    ...
    private float singleTextWidth;//单个中文字的宽度
    ...
    private void initResourcesIfNecessary()
    {
        ...
        if (itemsPaint == null)
        {
            ...
            singleTextWidth = (float) Math.ceil(Layout.getDesiredWidth("国", itemsPaint));
        }
	 ...
	 * @return
	 */
	private String getFixedString(String item) {
		if(TextUtils.isEmpty(item)){
			return item;
		}
		int width = getWidth();
		int chatCount= (int) (width / singleTextWidth);//在指定的宽度内能放多少个字
		if(chatCount > 0 && item.length() > chatCount){
			item = item.substring(0,chatCount-1)+"...";
		}
		return item;
	}
    ...
    private int calculateLayoutWidth(int widthSize, int mode)
    {
        ...
        if (maxLength > 0)
        {
            ...
            //向上取整  计算一个字符串宽度
            float textWidth = (float)Math.ceil(Layout.getDesiredWidth("国", itemsPaint));
