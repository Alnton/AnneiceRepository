package com.alnton.myFrameResource.view.slidingmenu;
...
import com.alnton.myFrameResource.view.slidingmenu.SlidingMenu.OnClosedListener;
import com.alnton.myFrameResource.view.slidingmenu.SlidingMenu.OnOpenedListener;
        ...
        switch (action & MotionEventCompat.ACTION_MASK)
        {
            ...
            case MotionEvent.ACTION_MOVE:
                if (!mIsBeingDragged)
                {
                    ...
                    final float y = MotionEventCompat.getY(ev, pointerIndex);
                    final float yDiff = Math.abs(y - mLastMotionY);
                    if ((xDiff > mTouchSlop || (mQuickReturn && xDiff > mTouchSlop / 4)) && xDiff > yDiff
                        && thisSlideAllowed(dx))
                    {
                        startDrag();
                        mLastMotionX = x;
