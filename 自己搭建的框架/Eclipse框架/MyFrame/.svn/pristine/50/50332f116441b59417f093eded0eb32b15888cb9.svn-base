...
public enum UpdateSoftUtil
{
     ...
     * 是否更新软件
     */
    public void isUpdateSoft(Context mContext)
    {
        this.context = mContext;
        
        JsonRequestParams params = new JsonRequestParams();
        params.put("verType", "0");
        HttpInterface.instance.onPostWithJson(mContext,
            Config.URLConfig.CHECK_SOFT_UPDATE_URL,
            params,
            new RequestCallback<UpdateVersion>(mContext, new TypeToken<ResponseEntity<UpdateVersion>>()
            {
            }.getType())
            {
                @Override
                public void onSuccess(UpdateVersion infoModel)
                {
                    super.onSuccess(infoModel);
                    updateVersion = infoModel;
                    if (null != updateVersion.getVerCode() && updateVersion.getVerCode().length() > 0)
                    {
                        if (!MyFrameCoreTools.getInstance().getVersionName(context).equals(updateVersion.getVerCode()))
                        {
                            alterDiolog();
                        }
                    }
