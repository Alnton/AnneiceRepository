...
public class ViewArea extends LinearLayout
{
    ...
    private MyImageView touchView;
    
    public ViewArea(Context context, Bitmap bitmap)
    {
        super(context);
        //这里的宽高要和xml中的LinearLayout大小一致，如果要指定大小。xml中 LinearLayout的宽高一定要用px像素单位，因为这里的宽高是像素，用dp会有误差！
        ...
        //这句就是我们的自定义ImageView
        touchView = new MyImageView(context, imgDisplayW, imgDisplayH);
        ...
        touchView.setLayoutParams(new LinearLayout.LayoutParams(layout_w, layout_h));//这是自定义imageView的大小，也就是触摸范围
        this.setGravity(17); // 17表示Center
